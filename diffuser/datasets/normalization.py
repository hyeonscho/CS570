...
class CDFNormalizer(Normalizer):
    ...
    def wrap(self, fn_name, x, verbose=False):
        shape = x.shape
        ## reshape to 2d
        ...
        for i, cdf in enumerate(self.cdfs):
            ...
            out[:, i] = fn(x[:, i], verbose=verbose)
        return out.reshape(shape)

    def normalize(self, x, verbose=False):
        return self.wrap("normalize", x)

    def unnormalize(self, x, verbose=False):
        return self.wrap("unnormalize", x, verbose)
...
class CDFNormalizer1d:
    ...
    def unnormalize(self, x, eps=1e-4, verbose=False):
        """
        X : [ -1, 1 ]
        """
        ## [ -1, 1 ] --> [ 0, 1 ]
