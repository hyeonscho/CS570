...
# Our Model
# self.model = diffuser.models.temporal.TemporalUnet
#
class GaussianInvDynDiffusion(nn.Module):
    def __init__(
        ...
        self.train_only_diffuser = train_only_diffuser
        if self.ar_inv:
    ...
    def get_loss_weights_fix(self, discount, cond):
        self.action_weight = 1
        dim_weights = torch.ones(self.observation_dim, dtype=torch.float32)
        ## decay loss with trajectory timestep: discount**t
        discounts = discount ** torch.arange(self.horizon, dtype=torch.float)
        ...
        loss_weights = torch.einsum("h,t->ht", discounts, dim_weights)
        if self.predict_epsilon:
            loss_weights = torch.as_tensor(cond[:, :, : self.observation_dim])
        return loss_weights
    ...
    def loss(self, x, cond, returns=None):
        if self.train_only_inv:
            ...
            loss, info = self.p_losses(x[:, :, self.action_dim :], cond, t, returns)
            info = {"a0_loss": loss}
