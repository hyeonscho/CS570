...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        loss_weights=None,
        condition=True,
        level_dim=0, # not used # to make it work for other configurations
    ):
        super().__init__()
        ...
        self.loss_fn = Losses[loss_type](loss_weights, self.action_dim, self.observation_dim)
        self.condition = condition
        self.sample_noise = True

    # wrapper for the original apply_conditioning -> so that it can be overriden in the child class
    def apply_conditioning(self, x, conditions): # maybe *args, **kwargs TODO
        return apply_conditioning(x, conditions, self.action_dim, self.observation_dim)
    ...
    def p_sample_loop(self, shape, cond, verbose=True, return_diffusion=False):
        ...
        if self.condition:
            ...
            x_sample = self.apply_conditioning(x_sample, cond)
