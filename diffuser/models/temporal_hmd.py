...
from .temporal import ResidualTemporalBlock, TemporalUnet
from diffuser.utils.debug import debug

class ResidualTemporalBlockHMD(ResidualTemporalBlock):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        
        self.cls_mlp = nn.Sequential(
            nn.Mish(),
            nn.Linear(kwargs['embed_dim'], args[1]),
    ...
    def forward(self, x, t, c):
        ...
        """
        out = self.blocks[0](x) + self.time_mlp(t) + self.cls_mlp(c)
        out = self.blocks[1](out)
        return out + self.residual_conv(x)

class TemporalUnetHMDLearnableCondition(nn.Module):
    def __init__(
        self,
        horizon,
        transition_dim,
        ...
        cls_dim = dims[1] # 32*1
        self.cls_token = nn.Parameter(torch.randn(1, cls_dim))
        ...
        mid_dim = dims[-1]
        self.mid_block1 = ResidualTemporalBlockHMD(
            mid_dim,
            mid_dim,
            embed_dim=time_dim,
            horizon=horizon,
        ...
        self.mid_block2 = ResidualTemporalBlockHMD(
            mid_dim,
            mid_dim,
            embed_dim=time_dim,
            horizon=horizon,
        ...
        self.final_conv = nn.Sequential(
            ...
            nn.Conv1d(dim_in, transition_dim+cond_dim, 1),
        )
    ...
    def forward(self, x, cond, time):
        ...
        c = einops.repeat(self.cls_token, '1 d -> b d', b = x.shape[0])
...
class TemporalUnetHMDLearnableConditionNoLevelOutput(TemporalUnetHMDLearnableCondition):
    def __init__(self, *args, **kwargs):
        kwargs['cond_dim'] = 0 # no level output
        super().__init__(*args, **kwargs)



# TODO:
class TemporalUnetHMDLearnableConditionV4(TemporalUnetHMDLearnableCondition):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        
        self.cls_cond_layer = nn.Linear(self.transition_dim*2+self.cls_token.shape[-1], self.cls_token.shape[-1])
        
    def dict2tensor(self, d):
        return torch.cat([v for k,v in d.items()], dim=-1)
    def forward(self, x, cond, time):
        """
        x : [ batch x horizon x transition ]
        """
        ...
        self.cls_cond_layer(torch.cat([self.dict2tensor(cond), c], dim=-1))
