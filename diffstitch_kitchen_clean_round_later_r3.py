...
Config.dynamic_model_path = (
    "/common/users/cc1547/projects/rainbow/diffstitch/dynamic/kitchen-partial-v0/mopo/"
...
with open("./clean_no_cluster_test/round1_stitch_kitchen_partial_H40.pkl", "rb") as f:
    stitched_round1_data = pickle.load(f)
stitched_trjs = []
for i in range(len(stitched_round1_data)):

    stitch_trj = dict(
        obs=stitched_round1_data[i][7][1:],
        act=stitched_round1_data[i][8][1:],
        rew=stitched_round1_data[i][9][1:],
        next_obs=stitched_round1_data[i][10][1:],
        dones=stitched_round1_data[i][11][1:],
        region_idx=stitched_round1_data[i][12],
    )

    traj2_idx = stitched_round1_data[i][2]
    traj2_segment_idx = stitched_round1_data[i][3]
    trj2 = env_dataset.get_full_info_traj(traj2_idx)
    for k in ["obs", "act", "rew", "next_obs", "dones"]:
        trj2[k] = trj2[k][traj2_segment_idx:]

    traj1_idx = stitched_round1_data[i][0]
    traj1_segment_idx = stitched_round1_data[i][1]
    trj1 = env_dataset.get_full_info_traj(traj1_idx)
    for k in ["obs", "act", "rew", "next_obs", "dones"]:
        trj1[k] = trj1[k][
            : traj1_segment_idx + horizon
...
stitch_round1_dataset = StitchDataset(stitched_trjs)
stitch_round1_buffer = StitchBuffer(horizon=Config.horizon)
for i in range(stitch_round1_dataset.num_traj):
    info = stitch_round1_dataset.get_full_info_traj(i)
    stitch_round1_buffer.insert_traj(info)
stitch_round1_buffer.finalize()

with open("./clean_no_cluster_test/round2_stitch_kitchen_partial_H40.pkl", "rb") as f:
    stitched_round2_data = pickle.load(f)
stitched_trjs = []
for i in range(len(stitched_round2_data)):

    stitch_trj = dict(
        obs=stitched_round2_data[i][7][1:],
        act=stitched_round2_data[i][8][1:],
        rew=stitched_round2_data[i][9][1:],
        next_obs=stitched_round2_data[i][10][1:],
        dones=stitched_round2_data[i][11][1:],
        region_idx=stitched_round2_data[i][12],
    )

    traj2_idx = stitched_round2_data[i][2]
    traj2_segment_idx = stitched_round2_data[i][3]
    trj2 = env_dataset.get_full_info_traj(traj2_idx)
    for k in ["obs", "act", "rew", "next_obs", "dones"]:
        trj2[k] = trj2[k][traj2_segment_idx:]

    traj1_idx = stitched_round2_data[i][0]
    traj1_segment_idx = stitched_round2_data[i][1]
    trj1 = stitch_round1_dataset.get_full_info_traj(traj1_idx)
    for k in ["obs", "act", "rew", "next_obs", "dones"]:
        trj1[k] = trj1[k][
            : traj1_segment_idx + horizon
        ]  # the last state in trj1 is the first state in stitch_trj
...
stitch_round2_dataset = StitchDataset(stitched_trjs)
stitch_round2_buffer = StitchBuffer(horizon=Config.horizon)
for i in range(stitch_round2_dataset.num_traj):
    info = stitch_round2_dataset.get_full_info_traj(i)
    stitch_round2_buffer.insert_traj(info)
stitch_round2_buffer.finalize()
...
while generated_transitions < 150000:

    trj1_info = stitch_round2_buffer.sample_batch_traj(
        stitch_batch_size, env_dataset, []
    )
    ...
    for j in range(0, stitch_batch_size):
        ...
        generated_transitions = (
            generated_transitions + return_info["obs"].shape[0] + 2 * horizon
        )
        if succ_cnt % 1 == 0:
...
pdb.set_trace()
with open("./clean_no_cluster_test/round3_stitch_kitchen_partial_H40.pkl", "wb") as f:
