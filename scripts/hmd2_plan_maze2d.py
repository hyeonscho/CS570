...
from diffuser.guides.policies import HMDPolicy2
import diffuser.datasets as datasets
import diffuser.utils as utils

import argparse

parser = argparse.ArgumentParser("hmd2", add_help=False)
parser.add_argument("--cfg", type=str)
parser.add_argument('--rpd', action='store_true') # if not provided, it is False
...
policy = HMDPolicy2(diffusion, dataset.normalizer, pairs, jumps, args.short_seq_len)
...
for i in range(n_samples):
    ...
    renderer.composite(join(args.savepath, f'plan{i}.png'), np.array(samples.observations), ncol=1)
