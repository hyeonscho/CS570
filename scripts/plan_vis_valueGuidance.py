...
args.savepath = args.savepath[:-1] + 'visual_value_guidance'
...
for i in range(n_samples):
    ...
    for t in range(max_planning_steps - 1):
        ...
        if t == 0:
            ...
            action_model_input = torch.cat([
                torch.tensor(sequence[t-1][None, ...], dtype=torch.float32).to(device),
                torch.tensor(sequence[t][None, ...], dtype=torch.float32).to(device),
                torch.tensor(sequence[t+1][None, ...], dtype=torch.float32).to(device),
            ]).reshape(1, -1)
        ...
        next_norm_obs = torch.from_numpy(normalize_pixels(next_observation, env_name)).float()[None, ...] # [64, 64, 3]
