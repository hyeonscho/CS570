...
import torch
import os
from mopo.common import util, functional
from mopo.models.ensemble_dynamics import EnsembleModel
from operator import itemgetter
from mopo.common.normalizer import StandardNormalizer
...
class TransitionModel:
    def __init__(
        ...
        **kwargs
        ...
        self.reward_dim = self.model.reward_dim
    ...
    def eval_data(self, data, update_elite_models=False):
        obs_list, action_list, next_obs_list, reward_list = itemgetter(
        ...
        predictions = functional.minibatch_inference(
            ...
            batch_size=10000,
            cat_dim=1,
        )  # the inference size grows as model buffer increases

        if self.reward_dim:
            groundtruths = torch.cat((delta_obs_list, reward_list), dim=1).to(
                util.device
            )
        else:
            groundtruths = delta_obs_list.to(util.device)
    ...
    def update(self, data_batch):
        obs_batch, action_batch, next_obs_batch, reward_batch = itemgetter(
            "observations", "actions", "next_observations", "rewards"
        )(data_batch)
        ...
        if self.reward_dim:
            groundtruths = torch.cat((delta_obs_batch, reward_batch), dim=-1).to(
                util.device
            )
        else:
            groundtruths = delta_obs_batch.to(util.device)
        ...
        train_transition_loss += 0.01 * torch.sum(
    ...
    def model_loss(self, predictions, groundtruths, mse_only=False):
        ...
        if self.inc_var_loss and not mse_only:
            ...
            mse_losses = torch.mean(
                torch.mean(torch.pow(pred_means - groundtruths, 2) * inv_var, dim=-1),
                dim=-1,
            )
    ...
    def predict(self, obs, act, deterministic=False):
        ...
        if deterministic:
            ...
            pred_diff_means = (
                pred_diff_means
                + np.random.normal(size=pred_diff_means.shape) * ensemble_model_stds
            )
        ...
        if self.reward_dim:
            next_obs, rewards = (
                pred_diff_samples[:, :-1] + obs,
                pred_diff_samples[:, -1],
            )
        else:
            next_obs = pred_diff_samples + obs
            rewards = np.zeros(1).astype(np.float32)
    ...
    def save_model_snapshot(self, idx):
        self.model_best_snapshots[idx] = deepcopy(
            self.model.ensemble_models[idx].state_dict()
        )
