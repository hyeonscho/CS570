...
def stitch(
    ...
    while generated_transitions < gen_size:
        ...
        for i in range(0, stitch_batch_size):
            ...
            while len(available_positions) == 0:
                candidates_trj2_info = data_buffer.sample_batch_traj(
                    sample_optim_batch, stitched_region
                )
            ...
            traj2_idx = candidates_trj2_info[available_positions[chosen_index]][
                "traj_idx"
            ]
            ...
            cond_p[i][end_index + 1 :, obs_dim:] = traj2_normed_obs[
                : horizon - best_pos
            ]
        ...
        for j in range(0, stitch_batch_size):
            ...
            stitched_discounted_reward = 0
            for k in range(return_info["rew"].shape[0] - 1, -1, -1):
                stitched_discounted_reward = (
                    stitched_discounted_reward * 0.99 + return_info["rew"][k]
                )
            if stitched_discounted_reward < 0.0:
    ...
    with open(f"./gym/round2_stitch_{Config.dataset}_H40.pkl", "wb") as f:
...
def main(**cfg):
    ...
    with open(f"./gym/round1_stitch_{Config.dataset}_H40.pkl", "rb") as f:
