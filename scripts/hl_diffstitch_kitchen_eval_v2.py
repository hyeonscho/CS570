...
def evaluate(Config, test_r, env_list, hl_dataset, ll_dataset, hl_trainer, ll_trainer):
    ...
    jump = Config.jump
    ll_horizon = Config.jump + 1
    device = Config.device
    # if Config.act_pad:
    hl_trans_dim = hl_trainer.model.observation_dim
    ...
    while sum(dones) < num_eval:
        ...
        hl_cond = np.ones(shape=(num_eval, hl_horizon, 2 * hl_trans_dim))
        hl_cond[:, :, hl_trans_dim:] = 0
        hl_cond[:, :1, :obs_dim] = 0
        hl_cond[:, :1, hl_trans_dim : hl_trans_dim + obs_dim] = state_normed
        ...
        normed_hl_states = hl_samples[:, :, :obs_dim]
        hl_states = hl_dataset.normalizer.unnormalize(normed_hl_states, "observations")
        norm_ll_states = ll_dataset.normalizer.normalize(hl_states, "observations")

        ll_cond = np.ones(shape=(num_eval, ll_horizon, 2 * obs_dim))
        ll_cond[:, :, obs_dim:] = 0
        ll_cond[:, :1, :obs_dim] = 0
        ll_cond[:, :1, obs_dim:] = norm_ll_states[:, :1, :obs_dim]
        ll_cond[:, -1:, :obs_dim] = 0
        ll_cond[:, -1:, obs_dim:] = norm_ll_states[:, 1:2, :obs_dim]
...
def main(**deps):
    ...
    hl_transition_dim = observation_dim

    if Config.act_pad:
        jump = Config.jump
        hl_transition_dim = observation_dim + jump * action_dim

    hl_model_config = utils.Config(
        Config.model,
        savepath="model_config.pkl",
        horizon=int(np.ceil(Config.horizon / Config.jump)),
        transition_dim=hl_transition_dim,
    ...
    hl_diffusion_config = utils.Config(
        ...
        horizon=int(np.ceil(Config.horizon / Config.jump)),
        observation_dim=hl_transition_dim,
    ...
    print(f"load ll planner model: {ll_loadpath}")
    ...
    hl_loadpath = os.path.join(hl_loadpath, f"state_800000.pt")
    print(f"load hl planner model: {hl_loadpath}")
