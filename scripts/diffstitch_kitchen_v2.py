...
def stitch(
    ...
    while generated_transitions < gen_size:
        ...
        trj1_info = data_buffer.sample_batch_traj(stitch_batch_size, [], uniform=True)
        ...
        for j in range(0, stitch_batch_size):
            ...
            trj1_idx = trj1_info[j]["traj_idx"]
            if (end_index == 0) and (trj1_idx + 1 == traj2_idx):
    ...
    with open(
        f"./kitchen/round1_stitch_{dataset_name}_H{Config.task_len}.pkl", "wb"
    ) as f:
