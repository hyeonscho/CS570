...
def load_config(config_path, update_args):
    ...
    default_args_module = importlib.import_module(
        relative_path_to_module_path(default_config_path)
    )
    overwrite_args_module = importlib.import_module(
        relative_path_to_module_path(config_path)
    )
    default_args_dict = getattr(default_args_module, "default_args")
    args_dict = getattr(overwrite_args_module, "overwrite_args")
    assert (
        type(default_args_dict) == dict
    ), "default args file should be default_args=\{...\}"
    ...
    if "common" in args_dict:
        for sub_key in args_dict:
            if type(args_dict[sub_key]) == dict:
                args_dict[sub_key] = merge_dict(
                    args_dict[sub_key], default_args_dict["common"], "common"
                )
    return args_dict
    ...
    for key in update_dict:
        ...
        if key not in source_dict:
            ...
            if type(update_dict[key]) == dict:
                source_dict[key] = merge_dict(
                    source_dict[key], update_dict[key], ignored_dict_name
                )
            else:
                print(
                    "updated {} from {} to {}".format(
                        key, source_dict[key], update_dict[key]
                    )
                )
                source_dict[key] = update_dict[key]
    return source_dict
