...
def get_act_cls(act_fn_name):
    ...
    if act_fn_name == "tanh":
        ...
        raise NotImplementedError(
            f"Activation functtion {act_fn_name} is not implemented. \
...
class MLPNetwork(nn.Module):
    ...
    def weights(self):
        return [
            ...
            if isinstance(net, torch.nn.modules.linear.Linear)
        ]
...
class EnsembleModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.max_logvar = nn.Parameter(
            (torch.ones((1, self.out_dim)).float() / 2).to(device), requires_grad=True
        )
        self.min_logvar = nn.Parameter(
            (-torch.ones((1, self.out_dim)).float() * 10).to(device), requires_grad=True
        )
    ...
    def predict(self, input):
        ...
        if len(input.shape) == 3:
            model_outputs = [
                net(ip) for ip, net in zip(torch.unbind(input), self.ensemble_models)
            ]
