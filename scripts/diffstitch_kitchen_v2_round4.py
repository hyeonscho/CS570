...
def stitch(
    ...
    with open(
        f"./kitchen/round4_stitch_{dataset_name}_H{Config.task_len}.pkl", "wb"
    ) as f:
...
def main(**cfg):
    ...
    with open(
        f"./kitchen/round2_stitch_{dataset_name}_H{Config.task_len}.pkl", "rb"
    ) as f:
        stitched_data_r2 = pickle.load(f)
    stitched_trjs = []
    for i in range(len(stitched_data_r2)):

        stitch_trj = dict(
            obs=stitched_data_r2[i][7][1:],
            act=stitched_data_r2[i][8][1:],
            rew=0.0 * stitched_data_r2[i][9][1:],
            next_obs=stitched_data_r2[i][10][1:],
            dones=stitched_data_r2[i][11][1:],
            region_idx=stitched_data_r2[i][12],
        )

        traj2_idx = stitched_data_r2[i][2]
        traj2_segment_idx = stitched_data_r2[i][3]
        trj2 = env_dataset.get_full_info_traj(traj2_idx)
        for k in ["obs", "act", "rew", "next_obs", "dones"]:
            trj2[k] = trj2[k][traj2_segment_idx:]

        traj1_idx = stitched_data_r2[i][0]
        traj1_segment_idx = stitched_data_r2[i][1]
    ...
    stitch_dataset_r2 = StitchDataset(stitched_trjs)
    stitch_buffer_r2 = StitchBuffer(horizon=Config.horizon)
    for i in range(stitch_dataset_r2.num_traj):
        info = stitch_dataset_r2.get_full_info_traj(i)
        stitch_buffer_r2.insert_traj(info)
    stitch_buffer_r2.finalize()

    with open(
        f"./kitchen/round3_stitch_{dataset_name}_H{Config.task_len}.pkl", "rb"
    ) as f:
        stitched_data_r3 = pickle.load(f)
    stitched_trjs = []
    for i in range(len(stitched_data_r3)):

        stitch_trj = dict(
            obs=stitched_data_r3[i][7][1:],
            act=stitched_data_r3[i][8][1:],
            rew=0.0 * stitched_data_r3[i][9][1:],
            next_obs=stitched_data_r3[i][10][1:],
            dones=stitched_data_r3[i][11][1:],
            region_idx=stitched_data_r3[i][12],
        )

        traj2_idx = stitched_data_r3[i][2]
        traj2_segment_idx = stitched_data_r3[i][3]
        trj2 = env_dataset.get_full_info_traj(traj2_idx)
        for k in ["obs", "act", "rew", "next_obs", "dones"]:
            trj2[k] = trj2[k][traj2_segment_idx:]

        traj1_idx = stitched_data_r3[i][0]
        traj1_segment_idx = stitched_data_r3[i][1]
        trj1 = stitch_dataset_r2.get_full_info_traj(traj1_idx)
        for k in ["obs", "act", "rew", "next_obs", "dones"]:
            trj1[k] = trj1[k][
                : traj1_segment_idx + Config.horizon
            ]  # the last state in trj1 is the first state in stitch_trj
    ...
    stitch_dataset_r3 = StitchDataset(stitched_trjs)
    stitch_buffer_r3 = StitchBuffer(horizon=Config.horizon)
    for i in range(stitch_dataset_r3.num_traj):
        info = stitch_dataset_r3.get_full_info_traj(i)
        stitch_buffer_r3.insert_traj(info)
    stitch_buffer_r3.finalize()
